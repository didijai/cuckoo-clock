        ...
        function fitClockToScreen() {
            ...
            // Bus Schedule Mode: measure the left-docked panel's real
            // footprint and re-center the clock inside [panel edge, stage edge].
            // Below 640px the panel becomes a full overlay instead, so no
            // space is reserved and the clock keeps its normal position.
            if (busScheduleEnabled && window.innerWidth >= 640) {
                const panelRect = busPanel.getBoundingClientRect();
                const stageRect = stage.getBoundingClientRect();
                const gap = 24; // breathing room between panel and clock
                const regionLeft = Math.max(panelRect.right + gap, stageRect.left);
        ...
        window.addEventListener('orientationchange', fitClockToScreen);
